...
export const processNotificationRequest = onDocumentCreated(
  ...
  async (
    ...
    const typedTargets = targets as unknown as (
      FunctionNotificationTarget[] | undefined
    );
    ...
    try {
      ...
      const allSentSuccessfully = response.failureCount === 0;
  ...
  async (request: CallableRequest<UpdateAdminAuthDetailsData>) => {
    ...
    try {
      ...
      return {
        success: true,
        message:
          "Admin authentication and Firestore details updated successfully.",
      };
    } catch (error) {
...
export const createAdminUser = onCall(
  ...
  async (request: CallableRequest<CreateAdminUserData>) => {
    ...
    try {
      ...
      const photoURL =
        `https://placehold.co/100x100.png?text=${name.substring(0, 2)
