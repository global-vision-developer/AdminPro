...
Энэхүү төсөл нь Next.js-ийн `src` директорын зохион байгуулалтыг дагаж мөрддөг. Гол хавтаснуудын үүрэg:
...
*   **`.env`:** (Эсвэл `.env.local`) Энэ файлд Firebase-ийн API түлхүүр зэрэг нууцлалтай, орчноос хамааралтай хувьсагчдыг хадгална. Энэ файл нь аюулгүй байдлын үүднээс Git-д commit хийгдэхгүй.
*   **`firebase.json`:** Firebase-ийн үйлчилгээнүүдийн (Firestore, Functions) тохиргоог агуулна. Жишээлбэл, Firestore-ийн дүрэм (`firestore.rules`) болон Cloud Function-ий эх код аль хавтаст байгааг зааж өгдөг.
*   **`next.config.js`:** Next.js аппликейшны үндсэн тохиргоог хийдэг файл. Жишээ нь, гаднын серверээс зураг ачаалах зөвшөөрөл (images.remotePatterns) зэргийг энд тодорхойлно.
*   **`package.json`:** Төслийн бүх хамааралтай сангууд (dependencies, devDependencies) болон түгээмэл ашиглагддаг тушаалуудыг (scripts) тодорхойлдог.
*   **`tailwind.config.ts`:** Tailwind CSS-ийн тохиргоо. Аппликейшны өнгөний схем, фонт, нэмэлт загваруудыг энд оруулна.
*   **`tsconfig.json`:** TypeScript хөрвүүлэгчийн тохиргоог агуулдаг. Энэ нь кодын чанарыг хянаж, алдааг багасгахад тусалдаг.
*   **`components.json`:** `shadcn/ui` компонентуудыг нэмэх, удирдах үед ашиглагддаг тохиргооны файл.
*   **`vercel.json`:** Төслийг Vercel платформ дээр байршуулахад зориулсан тохиргоог агуулна.
